...
{
    zmqpp::socket command_pull(zmq_ctx, zmqpp::socket_type::pull); // descarga actualizacion
    zmqpp::socket result_push(zmq_ctx, zmqpp::socket_type::push); //sube actualizacion
    command_pull.connect(imager_cmd_addr);// direccion donde se verifica la conexion
    result_push.connect(worker_ret_addr); // direccion donde sube la actualizacion
    zmqpp::poller poller; //flag
    poller.add(command_pull, zmqpp::poller::poll_in);
    unique_ptr<Image> grabbed(new Image);//puntero inteligente de la clase Image llamado grabbed

    if (!camdrv->initialize(0, 0, MODE_0, PIXEL_FORMAT_RAW12, HQ_LINEAR)) {
        debug_cerr << "Unable to initialize camera" << endl;
        return;
    }
    camdrv->set_trigger(false); //dispara el trigger de la camara como inactivo

    if (!camdrv->start()) {
        return;
    }
    camdrv->set_strobe(1, false); //dispara el control de la claridad de la señal digital como inactivo
...
bool handle_command(Json &cmd, fs::path &base_directory, shared_ptr<CameraDriver> camdrv)
{
    ...
    if (kind == "config") {
        ...
        if (in_capture) {
            camdrv->set_trigger(0, 0, 1); // colocar una configuraciones basicas, GPIO-camera
