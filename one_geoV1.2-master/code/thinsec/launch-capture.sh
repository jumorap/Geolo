...
main()
{
    ...
    stage_dir=${1:-"$HOME/Escritorio/one_geoV1.2/image/stage"}; shift
    gui_script=${1:-"$HOME/Escritorio/one_geoV1.2/code/thinsec/microscope/main_server.py"}; shift
    driver_script_dir=${1:-"$HOME/Escritorio/one_geoV1.2/code/pgr"}; shift
    ...
    sleep 2 && chromium "http://localhost:${local_port}" &
